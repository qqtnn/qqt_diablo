...
-- app name for console print
local app_name = "[HELLTIDE-MAIDEN-AUTO] "
-- app version
...
-- run core logic every 0.1 seconds
local update_interval = 0.1
-- tick rate for teleport
local last_teleport_time = 0
-- give teleport 25s wait time for loading screens (you may reduce this value if your loading screens are faster)
local teleport_interval = 20.0
-- see how much time is left
local teleport_elapsed = 0
...
local helltide_player_buff_id = 1066539
...
local helltide_tps = {
    ...
    {name = "Iron Wolves Encampment (internal name: Kehj_Oasis)", id = 0xDEAFC},
...
local local_console_print_message = ""
...
-- enable or disable exploring circle at helltide maiden
local draw_explorer_circle = 0
...
local insert_hearts_interval = 0
local insert_hearts_elapsed = 0
local last_insert_hearts_time = 0
...
local seen_enemies_interval = 0
local seen_enemies = 0
local last_seen_enemies_elapsed = 0
local insert_only_with_npcs_playercount = 0
-- fake mode and fake circle mode (simulating maiden at current player pos)
local fake_mode = 0
local fake_circle = 0
local fake_circle_completed = 0
local force_in_helltide_zone_was_on = 0
...
local function reset_helltide_maiden()
    ...
    teleport_elapsed = 0
    insert_hearts_elapsed = 0
    last_insert_hearts_time = 0
    insert_hearts_waiter = 0
    insert_hearts_waiter_elapsed = 0
    last_insert_hearts_waiter_time = 0
    fake_mode = 0
    fake_circle = 0
    fake_circle_completed = 0
    force_in_helltide_zone_was_on = 0
    seen_boss_dead = 0
    seen_enemies = 0
...
local function local_console_print(message)
    if menu.main_helltide_maiden_auto_plugin_debug:get() then
...
local function tp_to_next()
    -- local_console_print_message = "" .. app_name .. "tp_to_next() - called"
    -- local_console_print(local_console_print_message)
    -- do not jump when already jumping
    local current_time = os.clock()
    if current_time - last_teleport_time < teleport_interval then
        local_console_print_message = "" .. app_name .. "tp_to_next() - not jumping because of teleport_interval"
        local_console_print(local_console_print_message)
        helltide_maiden_auto_task = helltide_maiden_auto_tasks.IN_TELEPORT
        -- frame time of update is 0.1, running this 10 times (0.1) equals 1s
        teleport_elapsed = teleport_elapsed - 0.1
        return
    end
    ...
    for i in pairs(helltide_tps) do
        if i == helltide_tps_iter then
            local_console_print_message = "" .. app_name .. "tp_to_next() - teleporting to helltide_tps_iter = " .. helltide_tps_iter .. " Zone Name = " .. helltide_tps[i].name .. " ID = " .. helltide_tps[i].id .. ""
            local_console_print(local_console_print_message)
            helltide_tps_next_zone_name = helltide_tps[i].name
            -- save curent time and wait teleport_interval until next jump
            last_teleport_time = os.clock()
            teleport_elapsed = teleport_interval
...
local function maidenpos_load()
    ...
    -- local_console_print_message = "" .. app_name .. "maidenpos_load() - LOADING maidenpos for helltide zone = " .. helltide_zone_name .. ""
    -- local_console_print(local_console_print_message)
    local maidenpos_length = 0

    -- load fake waypoints in fake mode
    if fake_mode == 1 or fake_circle == 1 then
        -- dont do this perm for circle mode, which sticks to last player Pos
        if fake_mode == 0 then
            if fake_circle == 1 and fake_circle_completed == 0 then
                maidenpos = { }
                table.insert(maidenpos, player_position)
                fake_circle_completed = 1
            end
        else
            -- always set to latest player position in fake mode
            maidenpos = { }
            table.insert(maidenpos, player_position)
        end
        -- local_console_print_message = "" .. app_name .. "maidenpos_load() - FAKE MODE ON maidenpos waypoints set to player position"
        -- local_console_print(local_console_print_message)
    else
        ...
        if maidenpos_length == 0 then
            ...
            -- Iron Wolves Encampment (Kehj_Oasis)
            elseif helltide_zone_name == "Kehj_Oasis" then
                maidenpos = {
            ...
            elseif helltide_zone_name == "Step_South" then
                ...
                local_console_print_message = "" .. app_name .. "maidenpos_load() - ERROR no waypoints for helltide zone = " .. helltide_zone_name .. ""
                local_console_print(local_console_print_message)
            end
        end
    end
    ...
    if maidenpos_length > 0 then
        ...
        -- should not happen, but maybe didnt click on any fake mode yet
        maidenpos = { }
        table.insert(maidenpos, player_position)
        helltide_final_maidenpos = maidenpos[1]
        maidenpos_length = 1
        -- local_console_print_message = "" .. app_name .. "maidenpos_load() - LOADED backup length is = " .. maidenpos_length .. ""
        -- local_console_print(local_console_print_message)
    end
end
...
on_render_menu(function()
        ...
        local menu_name = "Mera-Helltide Maiden Auto v" .. app_version .. ""
        if not menu.main_tree:push(menu_name) then
            return
        end
        ...
        -- slider for teleporter_interavl
        menu.main_helltide_maiden_auto_plugin_teleport_interval_slider:render("Teleporter Loadscreen Time", "Adjust waiting time after teleporting depending on your load screen", 2)
        ...
        if menu.main_helltide_maiden_auto_plugin_insert_hearts:get() then
            menu.main_helltide_maiden_auto_plugin_insert_hearts_interval_slider:render("Insert timer", "Time in seconds after trying to insert heart", 2)
        ...
        menu.enable_loot_logic:render("Enable Loot Logic", "Enable or disable the loot logic feature")
        ...
        -- checkbox to debug force player being in a helltide zone
        menu.main_helltide_maiden_auto_plugine_force_inhelltide:render("Simulate Helltide Zone", "Forces currently being in a helltide zone even if no helltide is active. Useful to run script on the same zone but without active helltide.")
        if menu.main_helltide_maiden_auto_plugine_force_inhelltide:get() then
            -- checkbox to simulate current position being the helltide maiden position
            menu.main_helltide_maiden_auto_plugin_fake_maiden:render("Fake Maiden At Current Player Pos", "Fakes Maiden position at being player position allowing to farm random spots using same circle")
            -- checkbox to simulate current position being the helltide maiden position
            menu.main_helltide_maiden_auto_plugin_fake_circle:render("Fake Maiden At Last Player Pos", "Fakes circle at previous player position to allow for static positioning")
        end

    
        -- checkbox to debug after arriving at helltide maiden
        menu.main_helltide_maiden_auto_plugin_debug:render("Debug", "Print Console Debug Information")
...
on_update(function()
        ...
        -- check if drawing of explorer circle is enabled/disabled via ingame menu
        if menu.main_helltide_maiden_auto_plugin_show_explorer_circle:get() then
            draw_explorer_circle = 1
        else
            draw_explorer_circle = 0
        end

        -- check if fake mode or fake circle is enabled
        if menu.main_helltide_maiden_auto_plugin_fake_maiden:get() then
            fake_mode = 1
        else
            fake_mode = 0
        end
        if menu.main_helltide_maiden_auto_plugin_fake_circle:get() then
            fake_circle = 1
        else
            fake_circle = 0
            fake_circle_completed = 0
        ...
        -- configure teleport_interval depending on UI menu
        teleport_interval = menu.main_helltide_maiden_auto_plugin_teleport_interval_slider:get()
        ...
        -- configure inserts_hearts and insert_hearts_interval depending on UI menu
        if menu.main_helltide_maiden_auto_plugin_insert_hearts:get() then
            insert_hearts = 1
            insert_hearts_interval = menu.main_helltide_maiden_auto_plugin_insert_hearts_interval_slider:get()
            if menu.main_helltide_maiden_auto_plugin_insert_hearts_afterboss:get() then
                -- resets insert_hearts_elapsed after seen death of helltide maiden boss
                insert_hearts_afterboss = 1
            else
                insert_hearts_afterboss = 0
            end
            if menu.main_helltide_maiden_auto_plugin_insert_hearts_afternoenemies:get() then
                -- resets insert_hearts_elapsed after reaching seen_enemies_interval when no enemies are in range
                seen_enemies_interval = menu.main_helltide_maiden_auto_plugin_insert_hearts_afternoenemies_interval_slider:get()
            else
                seen_enemies_interval = 0
            end
        else
            insert_hearts = 0
            insert_hearts_interval = 0
            insert_hearts_afterboss = 0
            insert_only_with_npcs_playercount = 0
        end
        ...
        if menu.main_helltide_maiden_auto_plugin_reset:get() then
            local_console_print_message = "" .. app_name .. "on_update() - Resetting"
            local_console_print(local_console_print_message)
            reset_helltide_maiden()
            return
        end
        ...
        if buffs then
            ...
            for i, buff in ipairs(buffs) do
                if buff.name_hash == helltide_player_buff_id then
                    found_player_in_helltide_zone = 1
                end
                if buff.name_hash == 1924 then
            ...
            if found_player_is_mounted == 1 and player_in_helltide_zone == 1 and found_player_in_helltide_zone == 0 then
                ...
                local_console_print_message = "" .. app_name .. "on_update() - Player probably accidentally mounted in helltide zone, forcing active helltide zone"
                local_console_print(local_console_print_message)
            end

            -- if player was in helltide and is not mounted, reset the application
            -- potentially helltide has ended or player is in wrong zone somehow
            if found_player_is_mounted == 0 and player_in_helltide_zone == 1 and found_player_in_helltide_zone == 0 then
                -- dont reset in force helltide zone mode
                if not menu.main_helltide_maiden_auto_plugine_force_inhelltide:get() then
                    reset_helltide_maiden()
                end
            end

            if found_player_is_mounted == 0 then
        ...
        if helltide_maiden_auto_task ~= helltide_maiden_auto_tasks.IN_TELEPORT then
            ...
            if not distance_check_last_player_position or current_time - distance_check_last_time > 4.0 then
                -- local_console_print_message = "" .. app_name .. "on_update() - Distance-Check - 4s passed saving current player_position"
                -- local_console_print(local_console_print_message)
            ...
            if distance_check_distance < 1.0 then
                ...
                if distance_check_is_stuck_counter == 1 then
                    ...
                    -- local_console_print_message = "" .. app_name .. "on_update() - Distance-Check - Possible stuck, first time detected"
                    -- local_console_print(local_console_print_message)
                end
            else
                distance_check_is_stuck_counter = 0
            ...
            if distance_check_is_stuck_counter >= 55 then
                ...
                distance_check_is_stuck_counter = 0
                local elapsed = os.clock() - distance_check_is_stuck_first_time
                local_console_print_message = "" .. app_name .. "on_update() - Distance-Check - WARNING - Stuck threshold reached, enabling Is_Stuck - took: " .. elapsed .. " seconds to detect"
                local_console_print(local_console_print_message)
            end
        end

        -- check if debug enforce player in helltide zone is enabled
        if menu.main_helltide_maiden_auto_plugine_force_inhelltide:get() then
            force_in_helltide_zone_was_on = 1
            -- force being in helltide for debugging reasons, e.g. play script in same zone but helltide is disabled currently
            player_in_helltide_zone = 1
            -- force fake maiden on current player, if not already in circle mode
            if fake_circle == 1 then
                 -- one or another, cant have both, force some UI button pressing so user doesnt enable both
                 menu.main_helltide_maiden_auto_plugin_fake_maiden:set(false)
                 menu.main_helltide_maiden_auto_plugin_fake_circle:set(true)
            else
                 menu.main_helltide_maiden_auto_plugin_fake_maiden:set(true)
                 menu.main_helltide_maiden_auto_plugin_fake_circle:set(false)
            end
        else
            -- UI elements might be still enabled but invisible, manually disable them
            fake_circle = 0
            fake_mode = 0
            if force_in_helltide_zone_was_on == 1 then
                -- reset once after force in zone was enabled
                reset_helltide_maiden()
                force_in_helltide_zone_was_on = 0
            end
        end
        ...
        if player_in_helltide_zone == 1 then
            ...
            -- local_console_print_message = "" .. app_name .. "on_update() - Player IN helltide detected"
            -- local_console_print(local_console_print_message)
            ...
            if not explorer_points or explorer_circle_radius_prev ~= explorer_circle_radius or fake_mode == 1 or fake_circle == 1 then
                ...
                -- local_console_print_message = "" .. app_name .. "on_update() - LOADED positions in helltide_final_maidenpos circle"
                -- local_console_print(local_console_print_message)
            end
            ...
            if helltide_maiden_arrivalstate == 0 then
                -- proceed moving
                local_console_print_message = "" .. app_name .. "on_update() - Using pathfinder to walk to maiden"
                local_console_print(local_console_print_message)
                ...
                if helltide_final_maidenpos and helltide_zone_pin == 0 then
                    ...
                    -- local_console_print_message = "" .. app_name .. "on_update() - set_map_pin on final helltide_final_maidenpos"
                    -- local_console_print(local_console_print_message)
                    -- local_console_print_message = "" .. app_name .. "on_update() - set_map_pin X = " .. helltide_final_maidenpos:x() .. ""
                    -- local_console_print(local_console_print_message)
                    -- local_console_print_message = "" .. app_name .. "on_update() - set_map_pin Y = " .. helltide_final_maidenpos:y() .. ""
                    -- local_console_print(local_console_print_message)
                    -- local_console_print_message = "" .. app_name .. "on_update() - set_map_pin Z = " .. helltide_final_maidenpos:z() .. ""
                    -- local_console_print(local_console_print_message)
                    helltide_zone_pin = 1
                end
                ...
                if distance_check_is_stuck == 0 then
                    ...
                    if not pathfinder_nextpos then
                        local_console_print_message = "" .. app_name .. "on_update() - ERROR - Pathfinder cannot find next position from maidenpos"
                        local_console_print(local_console_print_message)
                        return
                    end
                    pathfinder_nextpos = utility.set_height_of_valid_position(pathfinder_nextpos)
                    pathfinder_prevpos = pathfinder_nextpos
                    -- local_console_print_message = "" .. app_name .. "on_update() - pathfinder_nextpos X = " .. pathfinder_nextpos:x() .. ""
                    -- local_console_print(local_console_print_message)
                    -- local_console_print_message = "" .. app_name .. "on_update() - pathfinder_nextpos Y = " .. pathfinder_nextpos:y() .. ""
                    -- local_console_print(local_console_print_message)
                    -- local_console_print_message = "" .. app_name .. "on_update() - pathfinder_nextpos Z = " .. pathfinder_nextpos:z() .. ""
                    -- local_console_print(local_console_print_message)
                    pathfinder.request_move(pathfinder_nextpos)
                else
                    -- run if being stuck
                    local_console_print_message = "" .. app_name .. "on_update() - WARNING - Pathfinder STUCK detected finding next best walkable position based on current player position"
                    local_console_print(local_console_print_message)
                    ...
                    if utility.is_point_walkeable_heavy(walkeable_pos) then
                        local_console_print_message = "" .. app_name .. "on_update() - Found get_positions_in_radius() around current player_position, walking to alternative waypoint"
                        local_console_print(local_console_print_message)
                        pathfinder.clear_stored_path()
                ...
                if helltide_final_maidenpos then
                    local distance_to_maiden = helltide_final_maidenpos:squared_dist_to_ignore_z(player_position)
                    if distance_to_maiden < (2.0 * 2.0) then
                        local_console_print_message = "" .. app_name .. "on_update() - Detected Player NEAR helltide maiden, setting helltide_maiden_arrivalstate"
                        local_console_print(local_console_print_message)
                ...
                -- local_console_print_message = "" .. app_name .. "on_update() - Player ARRIVED at helltide maiden"
                -- local_console_print(local_console_print_message)
                ...
                if insert_hearts == 1 and menu.main_helltide_maiden_auto_plugin_insert_hearts_onlywithnpcs:get() then
                    ...
                    if insert_only_with_npcs_playercount == 0 then
                        ...
                        -- local_console_print_message = "" .. app_name .. "on_update() - NO OTHER PLAYERS FOUND IN CIRCLE, disabling to put in any heart"
                        -- local_console_print(local_console_print_message)
                        insert_hearts = 0
                    end
                ...
                if insert_hearts == 1 then
                     -- set last_insert_hearts_time on current time, if we are here the first time we dont want to put in a heart directly
                    if last_insert_hearts_time == 0 and seen_boss_dead == 0 then
                        last_insert_hearts_time = current_time
                        insert_hearts_elapsed = insert_hearts_interval
                        last_seen_enemies_elapsed = 0
                        local_console_print_message = "" .. app_name .. "on_update() - First time reached Altar, dont put any heart in yet but save current time for next try"
                        local_console_print(local_console_print_message)
                    end

                    -- check if insert_hearts_interval was reached and insert_hearts_waiter isnt enabled
                    if insert_hearts_waiter == 0 then

                        -- if we see helltide boss dead and user enabled to directly put heart after it, reset the last_insert_hearts_time time
                        if insert_hearts_afterboss == 1 then
                            ...
                            for i, obj in ipairs(enemies) do
                                ...
                                local is_enemy = obj:is_enemy()
                                -- helltide maiden asset name
                                if is_dead == "Dead" and is_enemy and name == "s04_demon_succubus_miniboss" and seen_boss_dead == 0 then
                                    -- just do this once, the corpse is there for some longer time
                                    seen_boss_dead = 1
                                    local_console_print_message = "" .. app_name .. "on_update() - BOSS DEAD SEEN, resetting last_insert_hearts_time"
                                    local_console_print(local_console_print_message)
                        ...
                        -- check for enemies around helltide boss, count them and place heart after we see no enemies for particular time, reset the last_insert_hearts_time time
                        ...
                        if menu.main_helltide_maiden_auto_plugin_insert_hearts_afternoenemies:get() then
                            if last_seen_enemies_elapsed >= seen_enemies_interval then
                                local_console_print_message = "" .. app_name .. "on_update() - INSERTING HEART because timer of seen_enemies_interval reached while having active enemies of = " .. seen_enemies .. ""
                                local_console_print(local_console_print_message)
                        ...
                        if current_time - last_insert_hearts_time > insert_hearts_interval and insert_hearts_waiter == 0 then
                            local_console_print_message = "" .. app_name .. "on_update() - INSERTING HEART because timer of insert_hearts_interval reached (or forced by seen_enemies or seen_boss_dead)"
                            local_console_print(local_console_print_message)
                            last_insert_hearts_time = current_time
                            -- we might were forced to be here by manipulating insert_hearts_interval to 0, we must fix this back
                            insert_hearts_elapsed = menu.main_helltide_maiden_auto_plugin_insert_hearts_interval_slider:get()
                            -- insert logic
                            ...
                            -- not yet or just put something in
                            insert_hearts_elapsed = insert_hearts_elapsed - 0.1
                        end
                    else
                        ...
                        if current_time - last_insert_hearts_waiter_time > insert_hearts_waiter_interval then
                            local_console_print_message = "" .. app_name .. "tp_to_next() - WAITED ENOUGH TIME AFTER INTERACTION - Falling back to exploring"
                            local_console_print(local_console_print_message)
                            ...
                            local currenthearts = get_helltide_coin_hearts()
                            if currenthearts > 0 then
                                ...
                                if seen_enemies == 0 or seen_boss_dead == 1 then
                                    ...
                                    -- try interacting once with all of the altar as long as currenthearts doesnt change
                                    for _, actor in ipairs(actors) do
                                        ...
                                        if currenthearts >= old_currenthearts then
                                            -- altar asset name
                                            if name == "s04_smp_succuboss_altar_a_dyn" then
                                               local_console_print_message = "" .. app_name .. "on_update() - INTERACTING WITH ONE ALTER OF MAIDEN, TRYING TO INSERT ONE HEART (MIGHT FAIL)"
                                               local_console_print(local_console_print_message)
                                            ...
                                            -- we plugged one break up
                                            break
                                        end
                                   end
                               end
                            else -- no hearts
                                -- local_console_print_message = "" .. app_name .. "on_update() - WARNING - No hearts available to put into altar"
                                -- local_console_print(local_console_print_message)
                            end
                        end
                    end
                ...
                if run_explorer == 1 and insert_hearts_waiter == 0 then
                    -- local_console_print_message = "" .. app_name .. "tp_to_next() - RUN EXPLORER"
                    -- local_console_print(local_console_print_message)
                    ...
                    if explorer_points then
                            ...
                            for i, obj in ipairs(enemies) do
                                if obj:is_enemy() then
                                    ...
                                    if is_close then
                                        ...
                                        -- local_console_print_message = "" .. app_name .. "on_update() - Explorer found close distance enemy, walking to it"
                                        -- local_console_print(local_console_print_message)
                                        -- take first close enemy
                                        break
                                    else
                                        -- not in close range but in circle within
                                        -- local_console_print_message = "" .. app_name .. "on_update() - Explorer found far distance enemy, walking to it"
                                        -- local_console_print(local_console_print_message)
                                        close_enemy_pos = position
                                    end
                                end
                        ...
                        if not close_enemy_pos then
                            ...
                            if utility.is_point_walkeable_heavy(random_waypoint) then
                                -- local_console_print_message = "" .. app_name .. "on_update() - Explorer found random position within circle"
                                -- local_console_print(local_console_print_message)
                        ...
                        if explorer_go_next == 1 then
                            ...
                            if current_time - last_explorer_threshold_check < explorer_threshold then
                                -- local_console_print_message = "" .. app_name .. "on_update() - WARNING - Explorer thresholding..."
                                -- local_console_print(local_console_print_message)
                                return
                            end
                            ...
                            if not close_enemy_pos then
                                local_console_print_message = "" .. app_name .. "on_update() - ERROR - Explorer found no position to walk to"
                                local_console_print(local_console_print_message)
                                return
                            end
                            ...
                            if explorer_point then
                                ...
                                if distance_between_old_and_new < 3.0 then
                                    ...
                                    skip_new_point = 1
                                    -- local_console_print_message = "" .. app_name .. "on_update() - SKIPPING - Explorer suggested new position to close to previous position"
                                    -- local_console_print(local_console_print_message)
                                end
                            end
                            if skip_new_point == 0 then
                                 -- randomize threshold variation depending on UI max value given
                                explorer_thresholdvar = menu.main_helltide_maiden_auto_plugin_explorer_thresholdvar:get()
                                explorer_thresholdvar = math.random(0, explorer_thresholdvar)
                                ...
                                local_console_print_message = "" .. app_name .. "on_update() - Explorer walking next (t = " .. explorer_threshold .. " s = " .. slider_threshold .. " v = " .. explorer_thresholdvar .. ")"
                                local_console_print(local_console_print_message)
                            ...
                            local distance_between_player = player_position:dist_to(explorer_point)
                            if distance_between_player < 2.5 then
                                local_console_print_message = "" .. app_name .. "on_update() - Explorer reached prev waypoint moving next"
                                local_console_print(local_console_print_message)
                                explorer_go_next = 1
                            else
                                -- keep moving
            ...
            -- local_console_print_message = "" .. app_name .. "on_update() - Player NOT IN helltide zone"
            -- local_console_print(local_console_print_message)
...
on_render(function()
        ...
        if menu.main_helltide_maiden_auto_plugin_show_task:get() then
            ...
            -- print teleport_elapsed when teleporting
            if helltide_maiden_auto_task == helltide_maiden_auto_tasks.IN_TELEPORT then
                local teleport_elapsed_rounded_no_decimals = round(teleport_elapsed, 0)
                graphics.text_2d("" .. app_name .. "Current task: " .. helltide_maiden_auto_task .. " (time left: " .. teleport_elapsed_rounded_no_decimals .. " seconds)", txta_top_left_position, 13, color_red)
            else
                graphics.text_2d("" .. app_name .. "Current task: " .. helltide_maiden_auto_task, txta_top_left_position, 13, color_red)
            end
            ...
            if run_explorer_mode == run_explorer_modes.OFF then
                graphics.text_2d("" .. app_name .. "Current explorer mode: " .. run_explorer_mode, txtb_top_left_position, 13, color_white)
            else
                graphics.text_2d("" .. app_name .. "Current explorer mode: " .. run_explorer_mode .. " Next Position: " .. explorer_threshold_rounded_no_decimals .. "s (added random " .. explorer_thresholdvar_rounded_no_decimals .. "s)", txtb_top_left_position, 13, color_white)
            end

            local distance_check_distance_rounded_no_decimals = round(distance_check_distance, 0)
            graphics.text_2d("" .. app_name .. "Distance checker: " .. distance_check_distance_rounded_no_decimals .. "u Is_Stuck: " .. distance_check_is_stuck .. " Stuck Counter: " .. distance_check_is_stuck_counter, txtc_top_left_position, 13, color_white)

            graphics.text_2d("" .. app_name .. "Current helltide zone: " .. helltide_zone_name .. " Next teleporter zone: " .. helltide_tps_next_zone_name, txtd_top_left_position, 13, color_white)

            if insert_hearts == 1 then
                local insert_hearts_interval_rounded_no_decimals = round(insert_hearts_interval, 0)
                local seen_enemies_interval_rounded_no_decimals = round(seen_enemies_interval, 0)
                local insert_hearts_elapsed_rounded_no_decimals = round(insert_hearts_elapsed, 0)
                ...
                graphics.text_2d("" .. app_name .. "Inserting hearts: Enabled, next insert in: " .. insert_hearts_interval_rounded_no_decimals .. "s (playersinrange: ".. insert_only_with_npcs_playercount .. " afternoenmies: " .. seen_enemies_is_enabled .. " afterboss: " .. insert_hearts_afterboss .. ") Elapsed: " .. insert_hearts_elapsed_rounded_no_decimals .. "s", txte_top_left_position, 13, color_white)
                graphics.text_2d("" .. app_name .. "                                                (seen_enemies_interval: " .. seen_enemies_interval_rounded_no_decimals .. "s seen_enemies: " .. seen_enemies .." seen_elapsed: " .. last_seen_enemies_elapsed_rounded_no_decimals .. "s waiter_interval: " .. insert_hearts_waiter_interval .. "s start_waiter: " .. insert_hearts_waiter .." waiter_elapsed: " .. insert_hearts_waiter_elapsed_rounded_no_decimals .. "s)", txtf_top_left_position, 13, color_white)
           else
                graphics.text_2d("" .. app_name .. "Inserting hearts: Disabled / Or currently no player in range", txte_top_left_position, 13, color_white)
            end
            ...
            if helltide_maiden_auto_task == helltide_maiden_auto_tasks.ARRIVED then
                ...
                if not show_helper_text_time_up then
                    show_helper_text_time_up = current_time
                end
                -- show helper_text only for 20 seconds
                if current_time - show_helper_text_time_up < 20.0 then
                    ...
                    -- print 2D text at top_left_position using font-size 16 in color red
                    graphics.text_2d("Arrived at helltide maiden boss - Use Orbwalker now", help_middle_left, 40, color_red)
                    graphics.text_2d("1) Enable Orbwalker", help_middle_lefta, 40, color_red)
                    graphics.text_2d("2) Enable Orbwalker Block Movement", help_middle_leftb, 40, color_red)
                    graphics.text_2d("3) Enable Auto-Loot and Full-Clear Toggle", help_middle_leftc, 40, color_red)
                end
            end
        end

        -- draw exploring circle
        if draw_explorer_circle == 1 and run_explorer == 1 then
...
console.print(app_name, "Version: ", app_version, " - LOADED")